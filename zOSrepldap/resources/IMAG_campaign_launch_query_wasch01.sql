select
     concat('Product: '+ github.[EntitlementOwner1],
            '/ ' + github.[EntitlementOwner2] , 
            '/ Resmask: ' + github.[EntitlementName]) as 'ResName1',
     '-' as 'ResName2',
     case 
     ...
     (     
        select distinct R1.Application, R1.EntitlementOwner1, R1.EntitlementOwner2, R1.EntitlementName, R1.ContactEmail, R1.User_ID 
        from GITHUB_REVIEW R1 
        where R1.ContactEmail like ('wasch01@ca.com') 
          and R1.EntitlementOwner1 like ('Top Secret for z/VM')
     ) github
